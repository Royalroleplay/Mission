...
_value = 0;
{
	...
	if(ITEM_ILLEGAL(_var) isEqualTo 1) then {
		if(!(ITEM_SELLPRICE(_var) isEqualTo -1)) then {
			...
			FNC_SUB(_houseInvData,[-1]);
			FNC_SUB(_houseInvVal,(([_var] call life_fnc_itemWeight) * _val));
			FNC_ADD(_value,(_val * ITEM_SELLPRICE(_var)));
		};
	};
...
if(_value > 0) then {
	...
	FNC_ADD(life_atmbank,round(_value / 2));
